...
![Model types](spring-ai-concepts-model-types.jpg)
...
As an example of how counter-intuitive it can be to create an effective prompt (for example, contrasting with SQL), a [recent research paper](https://arxiv.org/abs/2205.11916) found that one of the most effective prompts you can use starts with the phrase, <q>Take a deep breath and work on this step by step.</q>
...
![Embeddings](spring-ai-embeddings.jpg)
...
![Tokens](spring-ai-concepts-tokens.png)
...
Also, asking <q>for JSON</q> as part of the prompt is not 100% accurate.
...
![Structured Output Converter Architecture](structured-output-architecture.jpg)

The [Structured output conversion](api/structured-output.adoc) employs meticulously crafted prompts, often necessitating multiple interactions with the model to achieve the desired formatting.
...
This approach is colloquially referred to as <q>stuffing the prompt.</q>
The Spring AI library helps you implement solutions based on the <q>stuffing the prompt</q> technique otherwise known as xref::concepts.adoc#concept-rag[Retrieval Augmented Generation (RAG)].

![Prompt stuffing](spring-ai-prompt-stuffing.jpg)
...
Spring AI greatly simplifies code you need to write to support [tool calling](api/tools.adoc).
...
1. Split the document into parts while preserving the semantic boundaries of the content.
...
When a user’s question is to be answered by an AI model, the question and all the <q>similar</q> document pieces are placed into the prompt that is sent to the AI model.
...
![The main sequence of actions for tool calling](tools/tool-calling-01.jpg)
