...
Spring AI exposes structured output directly on the [`ChatClient`](api/chatclient.adoc) fluent API through `.entity(...)`.
...
* [Schema Validation & Self-Correction](api/structured-output/validation.adoc) -- detect malformed output and retry automatically with `validateSchema()`.
* [Provider-Native Structured Output](api/structured-output/native.adoc) -- enforce the schema at the provider API level with `useProviderStructuredOutput()`.
* [Output Converters](api/structured-output/converters.adoc) -- the lower-level `StructuredOutputConverter` API, the built-in converters, and custom/non-JSON converters.
...
![Structured Output Basic Flow](structured-output-basic.png)
...
See [Schema Validation & Self-Correction](api/structured-output/validation.adoc) for how the retry loop works and how to customize it.
...
See [Provider-Native Structured Output](api/structured-output/native.adoc) for supported providers and limitations.
...
See [Output Converters](api/structured-output/converters.adoc).
...
| Model wraps JSON in markdown fences, or non-JSON format | Implement [`StructuredOutputConverter<T>`](api/structured-output/converters.adoc) and pass it to `.entity(...)` |
...
**📌 NOTE**\
The `StructuredOutputConverter` is not used for LLM [Tool Calling](api/tools.adoc), as this feature inherently provides structured outputs by default.
